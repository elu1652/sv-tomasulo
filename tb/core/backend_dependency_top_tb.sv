...
module backend_dependency_top_tb;
    ...
    always @(posedge clk) begin
        ...
        if (cdb_valid) begin
            ...
            case (broadcast_count)
                1: if (cdb_tag != TAG_W'(0) || cdb_value != XLEN'(30))
                       $fatal(1, "First broadcast must be ROB0 = 30");
                2: if (cdb_tag != TAG_W'(1) || cdb_value != XLEN'(35))
                       $fatal(1, "Second broadcast must be ROB1 = 35");
        ...
        if (commit_valid) begin
            ...
            case (commit_count)
                1: if (commit_tag != TAG_W'(0) ||
                       ...
                       $fatal(1, "First commit must be ROB0: R1 = 30");
                2: if (commit_tag != TAG_W'(1) ||
                       commit_dest_reg != REG_ADDR_W'(4) ||
                       commit_value != XLEN'(35))
                       $fatal(1, "Second commit must be ROB1: R4 = 35");
    ...
    initial begin
        $dumpfile("waves/backend_dependency_top_tb.vcd");
        $dumpvars(0, backend_dependency_top_tb);
        ...
        initialize_register(REG_ADDR_W'(5), XLEN'(5));
        ...
        $display("backend_dependency_top_tb PASSED");
        $finish;
    end

    initial begin repeat (70) @(posedge clk); $fatal(1, "backend_dependency_top_tb timed out"); end
