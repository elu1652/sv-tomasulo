...
module backend_writeback_tb;
    ...
    task automatic reset_dut;
        begin
            ...
            if (!rob_alloc_ready) begin
                $display("ERROR: ROB should be ready after reset");
                $fatal;
            end
    ...
    task automatic allocate_rob(
        ...
        begin
            ...
            // Important: alloc_tag is the current tail.
            // Capture it before the posedge advances the tail.
            #1;
    ...
    task automatic dispatch_ready_add_to_rs(
        ...
        begin
            ...
            $display(
                "SUCCESS: dispatched ADD %0d + %0d -> ROB%0d",
                src1_value,
                src2_value,
                dest_tag
            );
    ...
    initial begin
        ...
        $dumpfile("backend_writeback_tb.vcd");
        $dumpvars(0, backend_writeback_tb);

        $display("INFO: FU latency = %0d cycles", FU_LATENCY);

        reset_dut();

        // 1. Allocate destination ROB entry.
        allocate_rob(3'd1, tag0);

        // 2. Dispatch ready ADD into RS.
        dispatch_ready_add_to_rs(tag0, 32'd10, 32'd20);

        // 3. RS should issue to FU automatically.
        // 4. FU should complete after fixed latency.
        // 5. CDB should broadcast the tagged result.
        wait_for_cdb_result(tag0, 32'd30);

        // Give ROB one more clock edge to sample the CDB broadcast.
        @(posedge clk);
        #1;

        // 6. ROB should commit in order.
        commit_expect(tag0, 3'd1, 32'd30);

        if (rob_count !== '0) begin
            $display("ERROR: expected ROB count 0 after commit, got %0d", rob_count);
            $fatal;
        end

        if (rs_count !== '0) begin
            $display("ERROR: expected RS count 0 after issue, got %0d", rs_count);
            $fatal;
        end

        $display("RS + FU + CDB + ROB INTEGRATION TEST PASSED");
        $finish;
    end
