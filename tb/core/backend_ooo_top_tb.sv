...
module backend_ooo_top_tb;
    ...
    always @(posedge clk) begin
        ...
        if (cdb_valid) begin
            ...
            case (broadcast_count)
                1: if (cdb_tag != TAG_W'(1) || cdb_value != XLEN'(15))
                       $fatal(1, "Younger ADD ROB1 = 15 must broadcast first");
                2: if (cdb_tag != TAG_W'(0) || cdb_value != XLEN'(20))
                       $fatal(1, "Older MUL ROB0 = 20 must broadcast second");
        ...
        if (commit_valid) begin
            ...
            case (commit_count)
                ...
                2: if (commit_tag != TAG_W'(1) ||
                       ...
                       $fatal(1, "Second commit must be ROB1: R4 = 15");
    ...
    initial begin
        $dumpfile("waves/backend_ooo_top_tb.vcd");
        $dumpvars(0, backend_ooo_top_tb);
        ...
        dispatch_instruction(OP_ADD, REG_ADDR_W'(5), REG_ADDR_W'(6), REG_ADDR_W'(4));
        ...
        $display("backend_ooo_top_tb PASSED");
        $finish;
    end

    initial begin repeat (80) @(posedge clk); $fatal(1, "backend_ooo_top_tb timed out"); end
