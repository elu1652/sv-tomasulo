...
module backend_cross_fu_dependency_tb;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (alu_rs_issue_valid && alu_rs_issue_ready) begin
                if (!mul_result_seen) begin
                    $display(
                        "ERROR: dependent ADD issued before MUL result broadcast"
                    );
                    $fatal;
                end
            ...
            if (cdb_valid) begin
                ...
                if (cdb_tag == 2'd0) begin
                    mul_result_seen = 1'b1;
                end
            end
    ...
    initial begin
        ...
        $dumpfile("backend_cross_fu_dependency_tb.vcd");
        $dumpvars(0, backend_cross_fu_dependency_tb);

        $display(
            "INFO: cross-FU dependency test, ALU latency=%0d, MUL latency=%0d",
            ALU_LATENCY,
            MUL_LATENCY
        );
        ...
        // Younger dependent consumer:
        //
        // I1: ADD R4, R1, R5
        //
        // R1 is not available from the architectural register file.
        // Dispatch should store mul_tag as source 1's dependency.
        dispatch_instruction(
            OP_ADD,
            3'd4,
            3'd1,
        ...
        // The ADD must be waiting in the ALU RS.
        if (alu_rs_count !== 1) begin
            $display(
                "ERROR: expected one waiting ADD in ALU RS, got count %0d",
                alu_rs_count
            );
            $fatal;
        end

        // It must not issue while the multiply result is unavailable.
        repeat (2) begin
            @(posedge clk);
            #1;

            if (alu_rs_issue_valid) begin
                $display(
                    "ERROR: dependent ADD became issue-valid before MUL completed"
                );
                $fatal;
            end
        end
        ...
        $display(
            "SUCCESS: dependent ADD remained waiting for multiply result"
        );

        // MUL produces ROB0 = 20.
        wait_for_cdb_result(
            mul_tag,
            32'd20
        );

        // The CDB broadcast should wake the ADD's R1 operand.
        // The ALU RS should then issue:
        //
        // 20 + 7 = 27
        wait_for_cdb_result(
            add_tag,
        ...
        // Commit must remain in original program order.
        commit_expect(
            mul_tag,
            3'd1,
            32'd20
        ...
        $display(
            "BACKEND CROSS-FU DEPENDENCY TEST PASSED"
        );
