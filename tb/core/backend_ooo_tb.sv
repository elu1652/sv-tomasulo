...
module backend_ooo_tb;
    ...
    initial begin
        ...
        $dumpfile("backend_ooo_tb.vcd");
        $dumpvars(0, backend_ooo_tb);

        $display(
            "INFO: ALU latency=%0d, MUL latency=%0d",
            ALU_LATENCY,
            MUL_LATENCY
        );
        ...
        // Older, long-latency instruction:
        //
        // I0: MUL R1, R2, R3
        //     R1 = 4 * 5 = 20
        ...
        // Younger, short-latency instruction:
        //
        // I1: ADD R4, R5, R6
        //     R4 = 7 + 8 = 15
        dispatch_instruction(
        ...
        // The younger ADD should complete first.
        wait_for_cdb_result(
            add_tag,
            32'd15
        );

        // The ADD is ready, but it cannot commit because the older
        // multiply is still at the head of the ROB.
        if (rob_commit_valid) begin
            $display(
                "ERROR: younger ADD became commit-valid before older MUL"
            );
            $fatal;
        end

        $display(
            "SUCCESS: younger ADD completed but commit remained blocked"
        );

        // The older multiply completes later.
        wait_for_cdb_result(
            mul_tag,
            32'd20
        );

        // Despite the ADD writing back first, commit must follow
        ...
        $display(
            "BACKEND OUT-OF-ORDER INTEGRATION TEST PASSED"
        );
